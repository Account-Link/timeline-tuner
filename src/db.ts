...
export const userDb = {
   ...
   * @param twitterId - Twitter user ID
   */
  async isUserActive(twitterId: string): Promise<boolean> {
    try {
   ...
   */
  async storeUserPreferences(twitterId: string, preferences: string[]) {
    try {
      // First get user
      const user = await prisma.user.findUnique({
        where: { twitterId },
   ...
   * @param twitterId - Twitter user ID
   */
  async getUserPreferences(twitterId: string): Promise<string[]> {
    try {
      const user = await prisma.user.findUnique({
        where: { twitterId },
        include: {
